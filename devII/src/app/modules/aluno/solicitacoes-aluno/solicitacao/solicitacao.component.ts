...
export class SolicitacaoComponent {
  ...
  abrirEdicao() {
    if (this.solicitacao.editavel) {
      const docs: Observable<DocFile[]> =
        this.docslist.listarDocumentosPorSolicitarEstagioId(
          parseInt(this.solicitacao.id)
        );
      docs.subscribe(
        (docs) => {
          const dialogRef = this.dialog.open(AtualizarDocsComponent, {
            data: { docs: docs, solicitacaoId: this.solicitacao.id },
          });
          docs.forEach((doc) => {
            console.log(doc);
          });
        ...
        (error) => {
          ...
          this.toastService.showMessage(
            'Essa solicitação não tem documentos...'
          );
        },
        () => {
          //
      ...
      this.toastService.showMessage(
        'Essa solicitação não está aberta para edição..'
      );
    }
  }
  ...
  openDialog(msg?: boolean) {
    const dialogRef = this.dialog.open(DetalhesSolicitacaoComponent, {
      data: {
        ...
        observacaoOn: this.observacaoOn,
        msgOnly: msg,
      },
    });
  ...
  abrirDownload() {
    if (
      ...
      this.solicitacao.status.toLowerCase() == 'finalizado'
    ) {
      const docs: Observable<DocFile[]> =
        this.docslist.listarDocumentosPorSolicitarEstagioIdAssinados(
          parseInt(this.solicitacao.id)
        );
      let url : string = '';
      const urlDrive = this.docslist.pegarUrlDocDrive(parseInt(this.solicitacao.id)).subscribe(
        (urlDrive) => {
         url = urlDrive.urlDrive;
        }
      );
      docs.subscribe(
        (docs) => {
        ...
        (error) => {
          //EERO
          this.toastService.showMessage(
            'Essa solicitação não tem documentos assinados...'
          );
        },
        () => {
          //
  ...
  ngOnInit() {
    this.solicitacao.dataSolicitacao = this.formatarDataSolicitacao(
      this.solicitacao.dataSolicitacao
    );
  ...
  statusInfo(st: number): string {
    if (st == 1 && this.solicitacao.etapa >= '1') {
      if (
        this.solicitacao.status.toLocaleLowerCase() == 'indeferido' &&
        this.solicitacao.etapa == '1'
      ) {
  ...
  statusSolicitacao(): string {
    ...
    if (
      this.solicitacao.editavel  ||
      this.solicitacao.status.toLowerCase() == 'respondido'
    ) {
      return 'statusColor5';
    }
    if (
      this.solicitacao.status.toLowerCase() == 'Em análise' ||
      this.solicitacao.status === 'Em Andamento'
    ) {
      return 'statusColor2';
    }

    if (
      this.solicitacao.status.toLowerCase() == 'indeferido' ||
      this.solicitacao.status === 'Indeferido' ||
      this.solicitacao.status === 'negado'
    ) {
      return 'statusColor3';
    }
    return 'statusColor1';
